...
function Autocomplete( {
  ...
  // Return the autocomplete.
    ...
    <div className="nextcellent-autocomplete-content">
      ...
      {open &&
				<ul aria-live="polite" id="nextcellent-autocomplete-option-popup" className="nextcellent-autocomplete-options">
					{isLoading && internalOptions.lenght <= 0 &&
						<li className="loading" />
					}
					{!isLoading && internalOptions?.map( ( option, index ) =>
						<li id={`nextcellent-autocomplete-option-${index}`} tabIndex='-1' className="option"  onClick={optionSelect} key={ index } data-option={ index } >{option.label}</li>
					)}
				</ul>
			}
		</div>
