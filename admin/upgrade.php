...
//class NGG_Upgrade {
	...
	function ngg_upgrade() {
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '$wpdb->nggpictures'" ) ) {
			...
			if ( version_compare( $installed_ver, '1.8.3', '<' ) ) {
				...
				$booleans = array( 'deleteImg', 'swfUpload', 'usePermalinks', 'useMediaRSS', 'usePicLens', 'activateTags', 'thumbfix', 'imgBackup', 'imgAutoResize',
					'galImages', 'galShowSlide', 'galNoPages', 'galImgBrowser', 'galHiddenImg', 'galAjaxNav' );
				...
				$ngg_options['irAutoplayTimeout'] = 0;
			...
			throw new ErrorException( __( 'Could not find NextCellent Gallery database tables, upgrade failed!', 'nggallery' ) );
		}
	}
	...
	function output() {
		...
		<div class="wrap">
			...
			<br/>
			<a class="button button-primary" href="<?php echo $filepath; ?>&amp;upgrade=now"><?php _e( 'Start upgrade now', 'nggallery' ); ?></a>
		</div>
	<?php
	}
